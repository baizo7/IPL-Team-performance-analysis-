...
from ipl_analytics.services.data_cleaner import build_clean_dataset, generate_quality_report
...
def query_dataset_sql(sql_query: str, parquet_path: Optional[Path] = None) -> pd.DataFrame:
    """Execute high-speed SQL query using DuckDB engine on Parquet file."""
    target_path = parquet_path or (BASE_DIR / 'all_ipl_matches.parquet')
    if DUCKDB_AVAILABLE and target_path.exists():
        try:
            conn = duckdb.connect(database=':memory:')
            query_formatted = sql_query.replace("all_ipl_matches", f"'{str(target_path)}'")
            return conn.execute(query_formatted).df()
        except Exception as e:
            logging.warning(f"DuckDB query failed: {e}")
    return pd.DataFrame()
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    """Clean raw DataFrame using the data cleaning pipeline."""
...
def _guess_bowler_type(bowler_name) -> str:
    """Guess bowler type from name."""
    if pd.isna(bowler_name):
        return 'Unknown'
    name = str(bowler_name).strip().lower()
    ...
    if any(p in name for p in left_arm_wrist):
        return 'Left-Arm Wrist Spin'
    if any(p in name for p in left_arm_orthodox):
        return 'Left-Arm Orthodox'
    if any(p in name for p in right_arm_leg):
        return 'Right-Arm Leg Spin'
    if any(p in name for p in right_arm_off):
